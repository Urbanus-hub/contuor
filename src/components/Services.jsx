    ...
    <div className="bg-[#EFF2F1]">
      <div className="w-full lg:h-[75vh] h-auto bg-[#3B5D50] px-5 sm:px-10 md:px-16 lg:px-25 flex lg:flex-row flex-col items-center lg:justify-between justify-center py-10 lg:py-0 mb-5">
        ...
        <div className="lg:w-[35%] w-full flex flex-col gap-5 text-center lg:text-left">
          ...
          <p className="text-[#9DAEA2] mb-[24px] text-sm md:text-base">
            Experience personalized interior solutions, expert consultations,
            and seamless delivery. Our team is dedicated to helping you bring
            your vision to life—every step of the way.
          </p>
      ...
      <div className="w-full px-5 py-20 sm:px-10 md:px-16 lg:px-25 flex flex-col md:flex-row md:flex-wrap lg:flex-wrap gap-4 lg:gap-[30px] bg-[#EFF2F1]">
        {why.map((item, index) => (
          <div
            key={index}
            className="w-full md:w-[45%] lg:w-[22%] h-auto md:h-[30vh] relative flex flex-col p-4 mb-6 md:mb-0"
          >
            <div className="relative w-10 h-10 ml-4 mt-3 mb-5 bg-[#DCE5E4] rounded-full">
              <img
                src={item.image}
                alt=""
                className="absolute left-[-20px] top-[-7px] w-10 h-10"
              />
            </div>

            <div className="flex-col">
              <h3 className="text-[14px] md:text-base font-medium mb-[14px]">
                {item.title}
              </h3>
              <p className="text-[14px] md:text-base text-black/70 pr-3">
                {item.description}
              </p>
            </div>
          </div>
